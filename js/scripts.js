...
/* Página RESERVATION INFO*/
$(document).ready(function(){
    $('.label-menu-reservation li a:first').addClass('active'); /* Aqui estamos diciendo que le agregaremos la clase active, que es el color de fondo de los titulos de los tabs al estar sobre ellos */
    $('.content-reservation article').hide(); /* Ocultaremos por defecto todos los tabs */
    $('.content-reservation article:first').show(); /* Mostraremos solo el primer tab */

    $('.label-menu-reservation li a').click(function(){
        /* Cada que demos en cualquiera de los enlaces donde demos clic, le quitaremos la clase y se lo agregaremos al que le estamos dando clic */
        $('.label-menu-reservation li a').removeClass('active');
        $(this).addClass('active');

        $('.content-reservation article').hide(); /* Cada vez que hagamos clic en uno, todos los demas tabs se oculten */
